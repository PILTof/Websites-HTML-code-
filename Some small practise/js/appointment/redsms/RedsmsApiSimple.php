...
class RedsmsApiSimple
{
    ...
    public function testing($to, $text, $from)
    {
        return [
            "items" => [0 => [
                "status" => "created",
    ...
    protected function sendPost($url, $data = [])
    {
        ...
        $head = $this->getHeaders($data);
        curl_setopt($curlResource, CURLOPT_HTTPHEADER, $head);
        curl_setopt($curlResource, CURLOPT_RETURNTRANSFER, 1);
    ...
    protected function getHeaders($data = [])
    {
        ...
        $rnd = 7835;
        $ts = microtime() . $rnd;
        $ts = "0.26105800 16656440407835" . $rnd;

        $impl = implode('', $data);
        $md = $impl . $ts . $this->apiKey;
        $md = md5($md);
